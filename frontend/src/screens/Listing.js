...
import { getListingDetail } from '../actions/listingActions'

const Listing = ({match, history}) => {
    const dispatch= useDispatch()
    const detail= useSelector((state) => state.listingDetail)
    const { listing }= detail
    const listingRealtor= useSelector((state) => state.listingRealtor)
    const { realtor }= listingRealtor
  ...
  const { isAuthenticated  } = auth
    ...
    useEffect(()=>{
      ...
      dispatch(getListingDetail(match.params.id))
      //setPrice(numberWithCommas(listing.price))
    },[match.params, history, isAuthenticated,dispatch ])
